  ...
  async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
      ...
      if (intent.requesterId !== req.user!.id) {
        throw createError(
          "Not authorized to update this intent",
          403,
          "FORBIDDEN",
        );
      }
      ...
      if (intent.status !== "OPEN" && intent.status !== "BIDDING") {
        throw createError(
          "Cannot update intent in current status",
          400,
          "INVALID_STATUS",
        );
      }
...
router.delete(
  ...
  async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
      ...
      if (
        intent.requesterId !== req.user!.id &&
        !req.user!.roles.includes("ADMIN")
      ) {
        throw createError(
          "Not authorized to delete this intent",
          403,
          "FORBIDDEN",
        );
      }
...
router.post(
  ...
  async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
      ...
      if (intent.status !== "OPEN" && intent.status !== "BIDDING") {
        throw createError(
          "Cannot bid on intent in current status",
          400,
          "INVALID_STATUS",
        );
      }
      ...
      if (existingBid) {
        throw createError(
          "You have already placed a bid on this intent",
          400,
          "DUPLICATE_BID",
        );
      }
      ...
      if (!agent) {
        throw createError(
          "You need a deployed agent to bid on intents",
          400,
          "NO_AGENT",
        );
      }
...
router.post(
  ...
  async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
      ...
      if (intent.requesterId !== req.user!.id) {
        throw createError(
          "Only the requester can reject bids",
          403,
          "FORBIDDEN",
        );
      }
...
router.post(
  ...
  async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
      ...
      if (
        intent.selectedSolverId !== req.user!.id &&
        intent.requesterId !== req.user!.id
      ) {
      ...
      if (intent.status !== "EXECUTING" && intent.status !== "ASSIGNED") {
        throw createError(
          "Cannot complete intent in current status",
          400,
          "INVALID_STATUS",
        );
      }
