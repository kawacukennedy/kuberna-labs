...
router.post('/register', async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    if (existingUser) {
      throw createError('Email or wallet already registered', 400, 'USER_EXISTS');
    }
...
router.post('/login', authLimiter, async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
...
router.post('/web3-nonce', async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
...
router.post('/web3-login', async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const { web3Address, signature, message } = req.body;

    if (!web3Address || !signature || !message) {
      throw createError('Address, signature, and message are required', 400, 'MISSING_FIELDS');
    }
    ...
    if (!user) {
      throw createError('No account found for this wallet', 404, 'USER_NOT_FOUND');
    }

    // Verify the nonce in the message matches the stored nonce
    if (!user.web3Nonce || !message.includes(user.web3Nonce)) {
      throw createError('Invalid or expired nonce', 401, 'INVALID_NONCE');
    }
...
router.post('/web3-register', async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
