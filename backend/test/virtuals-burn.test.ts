...
describe('Virtuals $40 Burn - Opus 4.8', () => {
  test('Phase 1: 200 code writes', async () => {
    const p = Array.from({ length: 200 }, (_, i) => ({ s: SYS_CODE, u: `Write Solidity: ${TOPICS_CODE[i % 10]}. Full implementation, 200+ lines.`, t: 2000 }));
    await batch(p, 5);
    console.log(`Phase 1: $${totalCost.toFixed(2)} / ${totalCalls} calls`);
  });

  test('Phase 2: 200 designs', async () => {
    const p = Array.from({ length: 200 }, (_, i) => ({ s: SYS_DESIGN, u: `Design: ${TOPICS_DESIGN[i % 10]} protocol. Architecture, math, risk, fees, upgrades. Be detailed.`, t: 2000 }));
    await batch(p, 5);
  ...
  test('Phase 3: 200 completions', async () => {
    ...
    const p = Array.from({ length: 200 }, (_, i) => ({ s: SYS_COMPLETE, u: `Complete this contract:\n${snippets[i % 5]}`, t: 2000 }));
    await batch(p, 5);
  ...
  test('Phase 5: 100 deep analyses', async () => {
    const p = Array.from({ length: 100 }, (_, i) => ({
      s: SYS_ANALYZE,
      u: `Analyze: ${['Aave vs Compound rates for ETH', 'Uniswap V4 vs V3 capital efficiency', 'LayerZero vs Wormhole security', 'Curve stableswap invariant depth', 'EigenLayer restaking yields', 'Lido stETH depeg risk analysis', 'Arbitrum vs Optimism TVL composition', 'MEV trends 2026 market structure', 'Morpho blue efficiency vs Aave', 'Perp DEX volumes and funding rates'][i % 10]}. Include specific numbers, formulas, and actionable insights.`,
      t: 2000,
    }));
    await batch(p, 5);
