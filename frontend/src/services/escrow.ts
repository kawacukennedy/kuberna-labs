...
import { usePublicClient, useWalletClient, useWriteContract, useReadContract, useWaitForTransactionReceipt } from 'wagmi';
...
export function useEscrow(): UseEscrowReturn {
  ...
  const createEscrow = useCallback(async (params: CreateEscrowParams): Promise<Hash | null> => {
  ...
  const fundEscrow = useCallback(async (escrowId: `0x${string}`): Promise<Hash | null> => {
    if (!walletClient || !userAddress) {
      setError(new Error('Wallet not connected'));
      return null;
    }
  ...
  const assignExecutor = useCallback(async (
    escrowId: `0x${string}`, 
    executor: Address
  ): Promise<Hash | null> => {
    if (!walletClient || !userAddress) {
  ...
  const releaseFunds = useCallback(async (escrowId: `0x${string}`): Promise<Hash | null> => {
    if (!walletClient || !userAddress) {
      setError(new Error('Wallet not connected'));
      return null;
    }
  ...
  const getEscrow = useCallback(async (escrowId: `0x${string}`): Promise<EscrowData | null> => {
  ...
  const getEscrowStatus = useCallback(async (escrowId: `0x${string}`): Promise<EscrowStatus | null> => {
    if (!publicClient) return null;
    ...
    try {
      ...
      return statusMap[Number(statusNum)] || 'None';
    } catch (err) {
...
export function generateEscrowId(intentId: string, requester: Address, timestamp: bigint): `0x${string}` {
  return keccak256(encodePacked(['string', 'address', 'uint256'], [intentId, requester, timestamp]));
}
