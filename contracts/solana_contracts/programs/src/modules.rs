# Additional Kuberna Solana Modules
# This extends the core module to cover all 18 original contracts
...
impl Workshop { const INIT_SPACE: usize = 32 + (4 + 100) + 4 + 4 + (4 + 200) + 8 + 1; }
...
impl Course { const INIT_SPACE: usize = 32 + (4 + 64) + 8 + 1; }
...
pub struct CreateMultisig<'info> {
    ...
    #[account()] pub owners: Vec<AccountInfo<'info>>,
...
#[account] #[derive(InitSpace)]
pub struct Multisig { pub owners: Vec<Pubkey>, pub threshold: u8, pub last_executed: i64, pub bump: u8 }
impl Multisig { const INIT_SPACE: usize = (4 + 100) + 1 + 8 + 1; }
#[event] pub struct MultisigCreated { pub multisig: Pubkey }
#[event] pub struct TransactionExecuted { pub multisig: Pubkey }
...
pub mod kuberna_vesting {
    ...
    pub fn release_vesting(ctx: Context<ReleaseVesting>) -> Result<()> {
        ...
        let releasable = (v.total_amount * elapsed as u64) / total_duration as u64 - v.released;
        v.released += releasable;
        emit!(VestingReleased { vesting: v.key(), amount: releasable });
        Ok(())
    }
}
#[derive(Accounts)]
...
pub struct ReleaseVesting<'info> {
    #[account(mut, seeds = [b"vesting", beneficiary.key().as_ref()], bump = vesting.bump)]
    pub vesting: Account<'info, Vesting>,
...
impl Vesting { const INIT_SPACE: usize = 32 + 8 + 8 + 8 + 8 + 1; }
#[event] pub struct VestingCreated { pub vesting: Pubkey, pub beneficiary: Pubkey, pub total_amount: u64 }
#[event] pub struct VestingReleased { pub vesting: Pubkey, pub amount: u64 }
...
impl Proposal { const INIT_SPACE: usize = 32 + (4 + 200) + 8 + 8 + 1 + 1; }
...
impl PriceOracle { const INIT_SPACE: usize = 8 + 8 + 1; }
